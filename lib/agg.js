...
/**
 * Alias for getStartOfDay to match the API spec (BE-4)
 * @param {Date} date - The date to get the start of day for
 * @returns {Date} Date object set to midnight of the given day
 */
const dayStart = (date = new Date()) => getStartOfDay(date);

/**
 * Alias for getDaysAgo to match the API spec (BE-4)
 * @param {number} n - Number of days to go back
 * @returns {Date} Date object representing the date N days ago
 */
const daysAgo = (n) => getDaysAgo(n);

module.exports = {
  getStartOfDay,
  getStartOfWeek,
  getDaysAgo,
  // Export aliases for BE-4 spec compliance
  dayStart,
  daysAgo,
};
